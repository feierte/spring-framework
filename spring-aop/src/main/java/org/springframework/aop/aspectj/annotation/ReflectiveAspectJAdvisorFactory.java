...
public class ReflectiveAspectJAdvisorFactory extends AbstractAspectJAdvisorFactory implements Serializable {
	...
	public List<Advisor> getAdvisors(MetadataAwareAspectInstanceFactory aspectInstanceFactory) {
		...
		// 使用包装模式来包装我们的 MetadataAwareAspectInstanceFactory 构建为 MetadataAwareAspectInstanceFactory
		...
		for (Method method : getAdvisorMethods(aspectClass)) {
			...
			// 循环解析切面中的方法。将方法封装成 Advisor 。如果找不到@PointCut 的信息，则会返回 null。
	...
	public Advisor getAdvisor(Method candidateAdviceMethod, MetadataAwareAspectInstanceFactory aspectInstanceFactory,
		...
		// 1. 切点信息的获取。这里如果没有被 Aspect 系列注解(@Pointcut、@Around、@Before等)修饰会返回 null
		...
		// 2. 根据切点信息封装成增强器，实例化我们的切面Advisor对象
		// ReflectiveAspectJAdvisorFactory#getAdvisor 最终封装成了一个InstantiationModelAwarePointcutAdvisorImpl 返回。
		// 实际上，在 Aop 中所有的增强都由 Advisor 的实现类InstantiationModelAwarePointcutAdvisorImpl 统一封装。
	...
	public Advice getAdvice(Method candidateAdviceMethod, AspectJExpressionPointcut expressionPointcut,
		...
		// 判断标注在方法上的注解类型，根据不同的注解生成不同的 增强（Advice）
		...
		// 设置我们构建出来的通知对象的相关属性比如DeclarationOrder，在代理调用的时候，责任链顺序上会用
