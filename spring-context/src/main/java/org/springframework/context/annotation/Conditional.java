 ...
 * 配置类是指存在注解 @Componet、@Import、@ImportResource、@ComponentScan 或方法上存在 @Bean 的类，参见 {@link ConfigurationClassUtils#isConfigurationCandidate(AnnotationMetadata)}。
 * 应用中通常会指定一个 Spring 扫描 bean 的包名，Spring 将包中满足条件的类识别为配置类，然后对配置类进行解析，解析的结果可能包含新的配置类，Spring 反复处理，直到没有新的配置类。
 * 然后将所有的配置类注册为 Spring 中的 bean。@Conditional 的执行时机就包含在解析配置类和注册 bean 两大阶段。<br/>
 *
 * 如果我们希望在解析配置类时不进行 @Conditional 判断，在注册 bean 时再进行 @Conditional 判断，则需要使用 Condition 的子类 {@link ConfigurationCondition} 。
 */
